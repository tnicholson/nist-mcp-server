...
class TestPerformance:
    ...
    async def test_concurrent_control_requests(self):
        ...
        with patch.object(server.loader, 'load_controls', new_callable=AsyncMock) as mock_load, \
            ...
            tasks = [
                server.get_control_details("AC-1")
                for _ in range(50)
            ]
        ...
        for i in range(5000):
            control = {
                ...
                "parts": [
                    {
                        "name": "statement",
                        "prose": "This is a test control statement. " * 50  # Substantial content
                    },
                    {
                        "name": "guidance",
    ...
    async def test_json_serialization_performance(self):
        ...
        for i in range(100):
            large_control["parts"].append({
                "name": f"part-{i}",
                "prose": "This is a very long prose section that contains substantial content. " * 20
            })
