...
def run_integration_tests():
    ...
    tests = [
        ...
        ("python -c \"import asyncio; from src.nist_mcp.server import NISTMCPServer; asyncio.run(NISTMCPServer().loader.initialize())\"", "Server Initialization Test"),
    ]
...
def run_coverage_tests():
    ...
    tests = [
        ("pytest --cov=src/nist_mcp --cov-report=term-missing --cov-report=xml --cov-fail-under=70", "Coverage Tests"),
    ]
...
def main():
    """Main test runner"""
    parser = argparse.ArgumentParser(description="Run comprehensive tests for NIST MCP Server")
    parser.add_argument("--security", action="store_true", help="Run only security tests")
    parser.add_argument("--quality", action="store_true", help="Run only code quality tests")
    ...
    if not any([args.security, args.quality, args.integration, args.performance, args.coverage]):
        args.all = True
